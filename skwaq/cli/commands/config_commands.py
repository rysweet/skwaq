...
class ConfigCommandHandler(CommandHandler):
    ...
    def _display_config(self, config: Dict, prefix: str = "") -> None:
        ...
        for key, value in config.items():
            ...
            if isinstance(value, dict):
                ...
                if any(sensitive in full_key.lower() for sensitive in ['key', 'token', 'secret', 'password']):
                    display_value = f"{str(value)[:4]}****{str(value)[-4:]}" if value and len(str(value)) > 8 else "****"
                    console.print(f"  [yellow]{full_key}[/yellow]: [dim]{display_value}[/dim] [red](SENSITIVE)[/red]")
                else:
    ...
    async def _handle_set(self) -> int:
        ...
        with create_status_indicator("[bold blue]Updating configuration...", spinner="dots") as status:
            try:
    ...
    async def _handle_reset(self) -> int:
        ...
        with create_status_indicator("[bold blue]Resetting configuration...", spinner="dots") as status:
            try:
                reset_config()
    ...
    async def _handle_check(self) -> int:
        ...
        if "OpenAI API" not in " ".join(issues):
            with create_status_indicator("[bold blue]Testing OpenAI API connection...", spinner="dots") as status:
                try:
                    ...
                    if result:
                        ...
                        warning("Could not connect to OpenAI API. Check your configuration.")
                        if fix:
        ...
        if not any(issue.startswith("Neo4j") for issue in issues):
            with create_status_indicator("[bold blue]Testing Neo4j connection...", spinner="dots") as status:
                try:
                    ...
                    if connector.is_connected():
                        ...
                        warning("Could not connect to Neo4j database. Check your configuration.")
                        if fix:
    ...
    async def _fix_configuration(self, issues: List[str]) -> bool:
        ...
        if "OpenAI API key not configured" in issues or "OpenAI API endpoint not configured" in issues:
        ...
        if any(issue.startswith("Neo4j") for issue in issues):
            ...
            if "Neo4j URI not configured" in issues:
                uri = prompt_for_input("Enter Neo4j URI:", default="bolt://localhost:7687")
        ...
        if "OpenAI API key not configured" in issues and not config.get("openai", {}).get("api_key"):
            fixed = False
        
        if "OpenAI API endpoint not configured" in issues and not config.get("openai", {}).get("endpoint"):
            fixed = False
        
        if "Neo4j URI not configured" in issues and not config.get("database", {}).get("uri"):
            fixed = False
        
        if "Neo4j username not configured" in issues and not config.get("database", {}).get("username"):
            fixed = False
        
        if "Neo4j password not configured" in issues and not config.get("database", {}).get("password"):
            fixed = False
    ...
    async def _fix_openai_connection(self) -> None:
        ...
        api_key = prompt_for_api_key("OpenAI API", get_config().get("openai", {}).get("api_key"))
    ...
    async def _fix_neo4j_connection(self) -> None:
        ...
        uri = prompt_for_input(
            "Enter Neo4j URI:",
            default=get_config().get("database", {}).get("uri", "bolt://localhost:7687")
        )
