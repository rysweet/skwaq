...
class VersionCommandHandler(CommandHandler):
    ...
    async def handle(self) -> int:
        ...
        try:
            ...
            console.print(f"  API Configuration: [{'green' if has_openai_key else 'red'}]{'Configured' if has_openai_key else 'Not Configured'}[/{'green' if has_openai_key else 'red'}]")
        except Exception:
...
class GuiCommandHandler(CommandHandler):
    ...
    async def handle(self) -> int:
        ...
        # Launch GUI
        with create_status_indicator(f"[bold blue]Starting GUI server on {host}:{port}...", spinner="dots") as status:
            ...
            try:
                ...
                if not no_browser:
                    ...
                    console.print(f"Opening browser to [link=http://{host}:{port}]http://{host}:{port}[/link]")
                
                # Print instructions
                console.print()
                console.print(f"GUI is running at [link=http://{host}:{port}]http://{host}:{port}[/link]")
