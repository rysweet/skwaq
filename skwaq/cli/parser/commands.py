...
def register_repository_parser(parser: SkwaqArgumentParser) -> None:
    ...
    Args:
        parser: Main argument parser
    """
    repo_parser = parser.create_command_parser(
        "repo",
    ...
    add_parser = repo_subparsers.add_parser(
        "add",
        help="Add a local repository"
    )
    ...
    github_parser = repo_subparsers.add_parser(
        "github",
    ...
    github_parser.add_argument(
        "--branch", "-b",
        help="Branch to clone (defaults to default branch)"
    )
    ...
    # Delete repository
    delete_parser = repo_subparsers.add_parser(
        "delete",
...
def register_ingest_parser(parser: SkwaqArgumentParser) -> None:
    ...
    ingest_parser.add_argument(
        "source",
        help="Path to the source to ingest"
    )
...
def register_config_parser(parser: SkwaqArgumentParser) -> None:
    ...
    Args:
        parser: Main argument parser
    """
    config_parser = parser.create_command_parser(
        "config",
    ...
    set_parser.add_argument(
        "path",
        help="Configuration path (e.g., openai.api_key)"
    )
    ...
    check_parser = config_subparsers.add_parser(
        "check",
        help="Check configuration for required values and connectivity"
    )
...
def register_gui_parser(parser: SkwaqArgumentParser) -> None:
    ...
    gui_parser.add_argument(
        "--host",
        default="localhost",
        help="Host to bind the GUI server to"
    )
...
def register_workflow_parsers(parser: SkwaqArgumentParser) -> None:
    ...
    """
    # QA workflow
    qa_parser = parser.create_command_parser(
        "qa",
    ...
    inquiry_parser.add_argument(
        "--prompt", "-p",
        help="Initial prompt to start the inquiry"
    )
    
    # Tool workflow
    tool_parser = parser.create_command_parser(
        "tool",
        "Run external tool workflow"
    )
    ...
    tool_parser.add_argument(
        "--args", "-a",
        help="Tool-specific arguments (JSON format)"
    )
    
    # Vulnerability research workflow
    research_parser = parser.create_command_parser(
        "research",
        "Run vulnerability research workflow"
    )
