...
from typing import Any, Dict, List, Optional, Set, Type, TypeVar, Union, Callable, AsyncGenerator
import asyncio
import uuid
import time
...
class WorkflowTransition:
    ...
    def __init__(
        ...
        data_transformer: Optional[Callable[[Workflow, WorkflowContext], Dict[str, Any]]] = None,
        ...
        self.name = name or f"{from_workflow_type.__name__}To{to_workflow_type.__name__}"
...
class WorkflowChain:
    ...
    def start(
        ...
        **workflow_args
        ...
        if self.context and self.current_workflow:
            workflow_id = getattr(self.current_workflow, "workflow_id", str(uuid.uuid4()))
        ...
        logger.info(f"Started workflow chain {self.name} with {initial_workflow_type.__name__}")
    ...
    def find_next_transition(
        ...
        matching_transitions = [
            ...
            if t.from_workflow_type == current_type
        ]
    ...
    async def execute_next(self) -> Optional[Dict[str, Any]]:
        ...
        transformed_data = transition.transform_data(self.current_workflow, self.context)
        
        # Get workflow IDs for context recording
        from_workflow_id = getattr(self.current_workflow, "workflow_id", str(uuid.uuid4()))
        ...
        logger.info(f"Transitioned to {transition.to_workflow_type.__name__} in chain {self.name}")
        ...
        try:
            ...
            async for result in self.current_workflow.run():
                ...
                if is_final_update:
                    ...
                    if transition and getattr(transition, "transition_type", None) == TransitionType.MANUAL:
                        yield {
    ...
    def resume(self) -> None:
        ...
        if self._is_running or not self.current_workflow:
            logger.warning("Cannot resume workflow chain - already running or no current workflow")
            return
...
class WorkflowExecutionPlan:
    ...
    def set_entry_point(self, scenario: str, chain_name: str) -> 'WorkflowExecutionPlan':
