...
class ToolInvocationWorkflow(Workflow):
    ...
    async def setup(self) -> None:
        ...
        if self.repository_id and not self.repository_path:
            result = self.connector.run_query(
                "MATCH (r:Repository) WHERE id(r) = $repo_id " "RETURN r.path as path",
                {"repo_id": self.repository_id},
            )
            ...
            for result in data.get("results", []):
                ...
                parsed_result = {
                    ...
                    "confidence": confidence_map.get(
                        result.get("issue_confidence", ""), 0.5
                    ),
                    "remediation": result.get(
                        "more_info", "No remediation guidance provided"
                    ),
                }
        ...
        try:
            ...
            for result in data.get("results", []):
                ...
                parsed_result = {
                    ...
                    "remediation": result.get("extra", {})
                    .get("metadata", {})
                    .get("fix", "No remediation guidance provided"),
                }
            ...
            for line in raw_results.strip().split("\n"):
                ...
                try:
                    ...
                    parsed_result = {
                        ...
                        "line_number": result.get("SourceMetadata", {})
                        .get("Data", {})
                        .get("LineNumber", 0),
