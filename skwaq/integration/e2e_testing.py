        ...
        try:
            for i, step in enumerate(self.steps):
                ...
                try:
                    ...
                    step["error"] = str(e)
                    step["success"] = False
                    self.results["errors"].append(f"Step '{step['name']}' error: {str(e)}")
                    break
        finally:
            ...
            for assertion in self.assertions:
                try:
                    ...
                    self.results["assertions_failed"] += 1
                    self.results["errors"].append(f"Assertion '{assertion['name']}' error: {str(e)}")
            ...
            self.results["success"] = (
                self.results["steps_completed"] == len(self.steps) and
                self.results["assertions_failed"] == 0
            )
...
class E2ETestRunner:
    ...
    def register_scenario(self, scenario: Union[E2ETestScenario, AsyncE2ETestScenario]) -> None:
    ...
    def run_scenario(
        ...
        if results["success"]:
            ...
            if fail_fast:
                raise RuntimeError(f"Test scenario '{scenario_name}' failed: {results['errors']}")
    ...
    def generate_report(
        ...
        if format == "json":
            ...
            lines = ["# End-to-End Test Report", "", f"Generated: {datetime.now().isoformat()}", ""]
            ...
            for name, result in self.results.items():
                ...
                lines.append(f"| {name} | {status} | {result['steps_completed']}/{len(self.scenarios[name].steps)} | {assertions} | {time_s} |")
            ...
            lines.append("")
            lines.append(f"- Scenarios: {successful_scenarios}/{total_scenarios} passed")
            lines.append(f"- Steps: {successful_steps}/{total_steps} completed")
            lines.append(f"- Assertions: {successful_assertions}/{total_assertions} passed")
...
class PredefinedScenarios:
    ...
    def create_vulnerability_research_scenario() -> AsyncE2ETestScenario:
        ...
        scenario.add_assertion(
            ...
            condition=lambda results: 
                "Run vulnerability research workflow" in results 
    ...
    def create_code_analysis_scenario() -> E2ETestScenario:
        ...
        scenario.add_assertion(
            ...
            condition=lambda results: 
                "Analyze vulnerable code" in results 
                and any("SQL injection" in str(finding.get("vulnerability_type", "")).lower() 
