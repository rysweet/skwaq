...
class QueryOptimizer:
    ...
    def analyze_query(self, query: str) -> Dict[str, Any]:
        ...
        if "MATCH" in query and not "WHERE" in query:
            analysis["issues"].append("Query without WHERE clause may return too many results")
            analysis["suggestions"].append("Add appropriate WHERE clause to limit results")
            
        if "MATCH (n)" in query:
            analysis["issues"].append("Query matches all nodes without label, which is inefficient")
            analysis["suggestions"].append("Add specific node label, e.g., MATCH (n:Label)")
            
        if "OPTIONAL MATCH" in query and query.count("OPTIONAL MATCH") > 2:
            analysis["issues"].append("Multiple OPTIONAL MATCH patterns may slow down query")
            analysis["suggestions"].append("Consider breaking into multiple queries or simplifying")
    ...
    def optimize_query(self, query: str) -> str:
        ...
        optimized = query.replace("MATCH (n)", "// Add a label for better performance\nMATCH (n:Label)")
        ...
        if "MATCH" in optimized and not "USING INDEX" in optimized:
            ...
            optimized = optimized.replace(
                "MATCH (n:Label)", 
                "MATCH (n:Label) USING INDEX n:Label(property)"
            )
