...
class SystemIntegrationManager:
    ...
    def _verify_health(self) -> None:
        ...
        if "db_connector" in self._components:
            ...
            try:
                ...
                self._health_status["database"] = "healthy" if is_connected else "unavailable"
            except Exception:
        ...
        if "agent_registry" in self._components:
            ...
            self._health_status["registered_agents"] = len(agent_registry.get_all_agents())
        else:
            self._health_status["agent_registry"] = "error"
            
        # Overall system health
        if all(status == "healthy" for status in self._health_status.values() if isinstance(status, str)):
    ...
    def get_health_status(self) -> Dict[str, Any]:
        ...
        if "resource_manager" in self._components:
            ...
            self._health_status["resource_usage"] = resource_manager.get_resource_usage()
