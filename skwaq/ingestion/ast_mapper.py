...
class ASTFileMapper:
    ...
    async def map_ast_to_files(self, repo_node_id: int, 
                              file_nodes: Dict[str, int]) -> Dict[str, Any]:
        ...
        for ast_node in ast_nodes:
            ...
            for path, file_id in file_nodes.items():
                ...
                if (file_path.endswith(path) or 
                    path.endswith(file_path) or 
                    path.replace("\\", "/") == file_path.replace("\\", "/") or
                    path.split("/")[-1] == file_path.split("/")[-1]):
        ...
        if stats["unmapped_nodes"] > 0:
            logger.warning(f"Could not map {stats['unmapped_nodes']} AST nodes to file nodes")
