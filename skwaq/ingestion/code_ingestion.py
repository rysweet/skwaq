...
class RepositoryIngestor:
    ...
    async def _process_filesystem(
        ...
        self.connector.create_relationship(
            repo_id, root_dir_id, "HAS_ROOT_DIRECTORY"
        )
    ...
    async def _process_directory(
        ...
        for item in dir_path.iterdir():
            ...
            if item.is_dir():
                ...
                self.connector.create_relationship(
                    dir_node_id, subdir_node_id, "CONTAINS"
                )
                
                # Link repository to subdirectory
                self.connector.create_relationship(
                    repo_id, subdir_node_id, "HAS_DIRECTORY"
                )
            ...
            # Process files
                ...
                if include_patterns and not any(item.match(pattern) for pattern in include_patterns):
                    continue
                ...
                self.connector.create_relationship(
                    dir_node_id, file_node_id, "CONTAINS"
                )
                
                # Link repository to file
                self.connector.create_relationship(
                    repo_id, file_node_id, "HAS_FILE"
                )
    ...
    async def _generate_repo_summary(
        ...
        try:
            ...
            has_license = any(f.name.lower() == "license" or f.name.lower() == "license.md" for f in repo_dir.glob("*"))
            has_gitignore = (repo_dir / ".gitignore").exists()
            
            # Find build configuration files
            build_files = []
            for pattern in ["requirements.txt", "setup.py", "package.json", "Makefile", "CMakeLists.txt"]:
...
"""
    ...
    def _is_code_file(self, file_path: Path) -> bool:
        ...
        code_extensions = {
            ...
            ".xml", ".json", ".yaml", ".yml", ".r", ".scala", ".groovy", 
            ".pl", ".lua", ".m", ".mm"
        }
