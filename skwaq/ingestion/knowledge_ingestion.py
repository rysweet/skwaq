    ...
    async def chunk_markdown_document(
        ...
        for section in sections:
            ...
            if current_size > 0 and current_size + section_size > max_chunk_size:
                ...
                if section["header"] and section["header"] not in current_chunk["headers"]:
        ...
        for chunk in chunks:
            ...
            {chunk['content'][:3000]}
            """
            chunk["concepts"] = (await self.openai_client.get_completion(
...
"""
        ...
        logger.info(f"Successfully ingested document: {document_name} (ID: {document_id}) with {len(section_ids)} sections")
...
async def _extract_vulnerability_patterns(
    ...
    if pattern_result.strip() == "None" or "no clear vulnerability" in pattern_result.lower():
        return []
    ...
    try:
        ...
        logger.warning(f"Failed to parse vulnerability patterns from response: {pattern_result}")
        return []
                    ...
                    for description in problem_type.get("description", []):
                        if description.get("lang") == "en":
                            ...
                            if cwe_value.startswith("CWE-"):
                                ...
                                cwe_result = connector.run_query(cwe_query, {"cwe_id": cwe_id})
...
async def link_related_knowledge(min_similarity: float = 0.75) -> Dict[str, int]:
    ...
    """
    logger.info(f"Linking related knowledge with similarity threshold: {min_similarity}")
    connector = get_connector()
    relationship_counts = {
    ...
    RETURN count(*) as count
    """
    cwe_pattern_result = connector.run_query(cwe_pattern_query, {"threshold": min_similarity})
    ...
    RETURN count(*) as count
    """
    cve_pattern_result = connector.run_query(cve_pattern_query, {"threshold": min_similarity})
    if cve_pattern_result:
        relationship_counts["cve_to_pattern"] = cve_pattern_result[0]["count"]
    
    total_relationships = sum(relationship_counts.values())
    logger.info(f"Created {total_relationships} knowledge relationships: {relationship_counts}")
    ...
    if knowledge_dir:
        try:
            ...
            if knowledge_path.exists() and knowledge_path.is_dir():
                directory_results = await ingest_knowledge_directory(str(knowledge_path))
                stats["documents_ingested"] = directory_results["successful"]
                stats["document_sections"] = directory_results.get("sections", 0)
                stats["vulnerability_patterns"] = directory_results.get("vulnerability_patterns", 0)
            else:
