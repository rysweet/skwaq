...
class RepositoryHandler:
    ...
    def clone_repository(
        self, repo_url: str, branch: Optional[str] = None, depth: Optional[int] = None
    ) -> str:
    ...
    def get_repository_metadata(
        self, repo_path: str, repo_url: Optional[str] = None
    ) -> Dict[str, Any]:
                ...
                metadata.update(
                    {
                        ...
                        "created_at": (
                            ...
                            if github_repo.created_at
                            else None
                        ),
                        "updated_at": (
                            github_repo.updated_at.isoformat()
                            if github_repo.updated_at
                            else None
                        ),
                        "license": (
                            github_repo.license.name if github_repo.license else None
                        ),
                    }
                )
            ...
            for root, dirs, files in os.walk(repo_path):
                ...
                for file in files:
                    ...
                    if ext:
                        ...
                        stats["extension_counts"][ext] = (
                            stats["extension_counts"].get(ext, 0) + 1
                        )
    ...
    def cleanup(self) -> None:
        ...
        for temp_dir in self._temp_dirs:
            try:
                ...
                logger.error(
                    f"Failed to clean up temporary directory {dir_name}: {str(e)}"
                )
