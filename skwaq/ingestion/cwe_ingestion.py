...
class CWEProcessor:
    ...
    async def ingest_cwe_xml(self, xml_file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"CWE ingestion complete. Processed: {stats['processed_cwes']}, Categories: {stats['categories']}")
            return stats
    ...
    async def _process_categories(self, root: ET.Element, ns_prefix: str, stats: Dict[str, Any]) -> None:
        ...
        for category in categories:
            try:
                ...
                properties["embedding"] = await self.openai_client.get_embedding(embedding_text)
                ...
                logger.error(f"Error processing CWE category {category.get('ID', 'unknown')}: {e}")
                stats["failed_cwes"] += 1
                
        logger.info(f"Processed {stats['categories']} CWE categories")
    
    async def _process_weaknesses(self, root: ET.Element, ns_prefix: str, stats: Dict[str, Any]) -> None:
...
"""
                properties["summary"] = await self.openai_client.get_completion(summary_prompt, temperature=0.3)
                ...
                weakness_id = self.connector.create_node(
                    labels=[NodeLabels.CWE, NodeLabels.VULNERABILITY_PATTERN, NodeLabels.KNOWLEDGE],
                    properties=properties,
                )
                ...
                for i, example in enumerate(code_examples):
                    await self._process_code_example(example, weakness_id, cwe_id, i, ns_prefix)
                ...
                logger.error(f"Error processing CWE-{weakness.get('ID', 'unknown')}: {e}")
                stats["failed_cwes"] += 1
    ...
    async def _process_relationships(self, root: ET.Element, ns_prefix: str, stats: Dict[str, Any]) -> None:
        ...
        for weakness in weaknesses:
            try:
                ...
                if related_elem is not None:
                    for related in related_elem.findall(f"./{ns_prefix}Related_Weakness"):
                ...
                logger.error(f"Error processing relationships for CWE-{weakness.get('ID', 'unknown')}: {e}")
        ...
        for category_id, member_ids in self.category_weaknesses.items():
            try:
                ...
                logger.error(f"Error processing category relationships for CWE-{category_id}: {e}")
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(CWE_URL) as response:
                if response.status != 200:
                    raise RuntimeError(f"Failed to download CWE database: {response.status}")
