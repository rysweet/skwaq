...
class SchemaManager:
    ...
    def create_constraints(self) -> bool:
        ...
        for label, property_name in unique_constraints.items():
            ...
            result = self._connector.run_query(check_query, {"constraint_name": constraint_name})
            ...
            if not result:
                ...
                try:
                    ...
                    logger.error(f"Failed to create constraint on :{label}({property_name}): {e}")
                    success = False
            else:
    ...
    def create_indexes(self) -> bool:
        ...
        for label, properties in standard_indexes.items():
            for property_name in properties:
                ...
                result = self._connector.run_query(check_query, {"index_name": index_name})
                ...
                if not result:
                    ...
                    try:
                        ...
                        logger.error(f"Failed to create index on :{label}({property_name}): {e}")
                        success = False
                else:
    ...
    def create_vector_indexes(self) -> bool:
        ...
        for label, (property_name, dimension) in vector_indexes.items():
            ...
            try:
                ...
                if result:
                    logger.info(f"Created or verified vector index on :{label}({property_name})")
                else:
                    logger.warning(f"Failed to create vector index on :{label}({property_name})")
                    success = False
            except Exception as e:
                logger.error(f"Error creating vector index on :{label}({property_name}): {e}")
                success = False
    ...
    def initialize_schema_components(self) -> Dict[str, bool]:
        ...
        if all(results.values()):
            ...
            logger.warning(f"Schema initialization completed with issues in: {', '.join(failed)}")
