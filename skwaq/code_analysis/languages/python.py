...
class PythonAnalyzer(LanguageAnalyzer):
    ...
    def analyze_ast(self, file_id: int, content: str) -> List[Finding]:
        ...
        sql_patterns = [
            # Raw SQL queries
            re.compile(
                r'execute\s*\(\s*f["\']SELECT|UPDATE|INSERT|DELETE.+\{([^}]+)\}',
                re.MULTILINE | re.IGNORECASE,
            ),
            re.compile(
                r'execute\s*\(\s*["\']SELECT|UPDATE|INSERT|DELETE.+\%\(|\%s|\$\d+',
                re.MULTILINE | re.IGNORECASE,
            ),
            # Django raw queries
            re.compile(
                r'raw\s*\(\s*f["\']SELECT|UPDATE|INSERT|DELETE',
                re.MULTILINE | re.IGNORECASE,
            ),
        ]
        ...
        cmd_patterns = [
            re.compile(
                r'subprocess\.(?:call|run|Popen)\s*\(\s*(?:f["\']|[\'"]\s*\+\s*|[\'"]\s*\.format)',
                re.MULTILINE,
            ),
            re.compile(
                r'os\.(?:system|popen|exec[lv][ep]?)\s*\(\s*(?:f["\']|[\'"]\s*\+\s*|[\'"]\s*\.format)',
                re.MULTILINE,
            ),
        ]
