...
# Registry instances for testing isolation
_registry_instances: Dict[str, "VulnerabilityPatternRegistry"] = {}


def reset_registry() -> None:
    """Reset the pattern registry - primarily for testing.

    This function clears all stored registry instances.
    """
...
class VulnerabilityPatternRegistry:
    ...
    async def register_pattern(
        ...
        if pattern_id is None:
            raise RuntimeError(
                f"Failed to create vulnerability pattern node for {name}"
            )
            ...
            for i, example in enumerate(examples):
                ...
                if example_id is not None:
                    ...
                    self.connector.create_relationship(
                        pattern_id, example_id, RelationshipTypes.HAS_EXAMPLE
                    )
        ...
        for cwe in cwe_nodes:
            ...
            for language, regex in regex_patterns.items():
                pattern_id = await self.register_pattern(
                    ...
                    cwe_id=cwe["cwe_id"],
                    examples=[
                        ex for ex in code_examples if ex.get("language") == language
                    ],
                )
                pattern_ids.append(pattern_id)

        logger.info(
            f"Generated {len(pattern_ids)} vulnerability patterns from CWE database"
        )
        return pattern_ids

    async def _generate_regex_patterns(
        self, name: str, description: str, examples: List[Dict[str, str]]
    ) -> Dict[str, str]:
...
"""
            ...
            try:
                ...
                regex_pattern = await self.openai_client.get_completion(
                ...
                try:
                    ...
                    logger.warning(
                        f"Invalid regex pattern generated for {name} ({language}): {regex_pattern}"
                    )
                    ...
                    if regex_pattern.startswith("/") and regex_pattern.endswith("/"):
                        ...
                        try:
                            ...
                            logger.error(
                                f"Could not fix regex pattern for {name} ({language})"
                            )

            except Exception as e:
                logger.error(
                    f"Error generating regex pattern for {name} ({language}): {e}"
                )
