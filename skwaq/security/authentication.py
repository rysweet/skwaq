...
class AuthEvent(SystemEvent):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            message=message or f"Authentication {action}: {'success' if success else 'failed'} for user {user_id}",
            target=None,
...
class AuthenticationManager:
    ...
    def _get_or_create_credentials_key(self) -> bytes:
        ...
        if not key_str:
            ...
            key_str = key.decode()
            config.set("security.credentials_key", key_str, source="authentication_manager")
    ...
    def _load_users(self) -> None:
        ...
        try:
            config = get_config()
            users_home = config.get("security.users_file", os.path.expanduser("~/.skwaq/users.json"))
    ...
    def _save_users(self) -> None:
        ...
        try:
            ...
            user_data = {username: user.to_dict() for username, user in self._users.items()}
    ...
    def authenticate(self, username: str, password: str, source_ip: Optional[str] = None) -> Optional[UserCredentials]:
        ...
        if user.password_hash != password_hash:
            ...
            if user.failed_attempts >= self._max_failed_attempts:
                ...
                logger.warning(f"Account locked for {self._lockout_period}s: {username} (failed attempts: {user.failed_attempts})")
    ...
    def authenticate_api_key(self, api_key: str, source_ip: Optional[str] = None) -> Optional[UserCredentials]:
    ...
    def validate_token(self, token: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if payload.get("jti") not in self._tokens:
                ...
                # (e.g., after a restart)
                logger.warning(f"Valid token not found in token store: {payload.get('jti')}")
    ...
    def revoke_token(self, token: str) -> bool:
        ...
        try:
            # Decode the token
            payload = jwt.decode(token, self._token_secret, algorithms=["HS256"], options={"verify_exp": False})
