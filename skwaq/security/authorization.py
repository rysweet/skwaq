...
class AuthEvent(SystemEvent):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            message=message or f"Authorization {action}: {'granted' if success else 'denied'} for user {user_id} ({permission.value})",
            target=None,
...
class Authorization:
    ...
    def has_permission(
        self, user: UserCredentials, permission: Permission, resource_id: Optional[str] = None
    ) -> bool:
...
def require_permission(permission: Permission, resource_id_arg: Optional[str] = None) -> Callable[[F], F]:
    ...
    def decorator(func: F) -> F:
        ...
        def wrapper(*args: Any, **kwargs: Any) -> Any:
            ...
            if user is None:
                raise AuthorizationError("User credentials not found in function arguments")
...
def get_resource_permissions(resource_type: str, resource_id: str) -> Dict[AuthRole, List[Permission]]:
