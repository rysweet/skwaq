...
class Sandbox:
    ...
    def __init__(
        ...
        self.working_dir = working_dir or Path(
            tempfile.mkdtemp(prefix="skwaq_sandbox_")
        )
    ...
    def _execute_in_container(
        ...
        try:
            ...
            execution_logs.append(
                f"Executing command in container: {' '.join(command)}"
            )
            ...
            docker_command = [
                ...
                container_name,
                "-v",
                f"{input_dir}:/sandbox/input:ro",  # Mount input as read-only
                "-v",
                f"{output_dir}:/sandbox/output:rw",  # Mount output as writable
                "--workdir",
                "/sandbox/input",
                "--network",
                "none" if not self.resource_limits.network_access else "bridge",
                "--memory",
                f"{self.resource_limits.memory_mb}m",
                "--memory-swap",
                f"{self.resource_limits.memory_mb}m",  # No swap
                "--cpus",
                "1",
                "--pids-limit",
                str(self.resource_limits.process_count),
                "--security-opt",
                "no-new-privileges",
