...
class ComplianceManager:
    ...
    def _load_built_in_requirements(self) -> None:
        ...
        auth_req = ComplianceRequirement(
            ...
            parameters={"min_length": 8, "require_numbers": True, "require_special": True},
        ...
        logger.info(f"Loaded {len(self._requirements)} built-in compliance requirements")
    ...
    def _register_built_in_validations(self) -> None:
        ...
        def validate_password_complexity(params: Dict[str, Any]) -> Tuple[bool, str]:
            ...
            if config_min_length is None or config_min_length < min_length:
                return False, f"Password minimum length ({config_min_length or 'not set'}) does not meet requirement ({min_length})"
        ...
        def validate_vuln_scanning(params: Dict[str, Any]) -> Tuple[bool, str]:
            ...
            try:
                ...
                if days_since_scan > min_frequency:
                    return False, f"Last vulnerability scan was {days_since_scan} days ago (requirement: {min_frequency} days)"
                
            except Exception as e:
                return False, f"Error parsing last scan date: {e}"
            
            return True, f"Vulnerability scanning was performed {days_since_scan} days ago"
    ...
    def add_requirement(self, requirement: ComplianceRequirement) -> None:
        ...
        logger.info(f"Added compliance requirement: {requirement.id} - {requirement.name}")
    ...
    def validate_requirement(
        ...
        validation_function = self._validation_functions.get(requirement.validation_function)
        if not validation_function:
            logger.error(f"Unknown validation function: {requirement.validation_function}")
            return False, None
        ...
        try:
            ...
            logger.error(f"Error validating compliance requirement {requirement_id}: {e}")
            return False, None
