...
class DebatePattern:
    ...
    async def execute(
        ...
        async def _handle_debate_argument(event: DebateArgumentEvent) -> None:
            ...
            current_round_args = sum(1 for (r, rn) in arguments.keys() if rn == round_num)
            ...
            if current_round_args >= expected_args_for_round:
                ...
                round_data = {
                    ...
                    "arguments": {
                        ...
                        } for arg in arguments.values() if arg.round_number == round_num
                    }
                }
        ...
        if mediator:
            mediator.register_event_handler(DebateArgumentEvent, _handle_debate_argument)
        ...
        try:
            ...
            for round_num in range(1, self.max_rounds + 1):
                ...
                await self._initiate_round(
                    proponent, opponent, mediator, 
                    round_num, debate_id, topic, 
                    debate_structure
                )
            ...
            if not debate_structure["completed"]:
                try:
                    await asyncio.wait_for(debate_complete.wait(), 10.0)  # Short timeout
                except asyncio.TimeoutError:
                    # Force conclusion
                    debate_structure["completed"] = True
            ...
            logger.info(f"Debate completed with {len(debate_structure['rounds'])} rounds")
            
            return debate_structure
        finally:
            # Clean up event handlers
            proponent.deregister_event_handler(DebateArgumentEvent, _handle_debate_argument)
            opponent.deregister_event_handler(DebateArgumentEvent, _handle_debate_argument)
            if mediator:
                mediator.deregister_event_handler(DebateArgumentEvent, _handle_debate_argument)
