...
"""
    ...
    async def _start(self):
        ...
        logger.info(
            f"Orchestrator agent started with {len(self.available_agents)} available agents"
        )
    ...
    def _discover_agents(self):
        ...
        # Filter out self
        other_agents = [
            agent for agent in all_agents if agent.agent_id != self.agent_id
        ]
        ...
        for agent in other_agents:
            ...
            logger.debug(
                f"Discovered agent: {agent.name} with capabilities: {capabilities}"
            )
    ...
    async def _execute_vuln_assessment_workflow(self, workflow_id: str):
        ...
        if not repository:
            raise ValueError(
                "Repository parameter is required for vulnerability assessment"
            )
    ...
    async def _handle_communication(self, event: AgentCommunicationEvent) -> None:
        ...
        if event.message_type == "status_update":
            ...
            if agent_id in self.available_agents:
                self.available_agents[agent_id].context.metadata[
                    "status"
                ] = event.message
            ...
            try:
                ...
                if "capabilities" in capability_data:
                    ...
                    logger.debug(
                        f"Updated capabilities for agent {agent_id}: {capability_data['capabilities']}"
                    )
            except Exception as e:
    ...
    async def _handle_task_request(self, agent_id: str) -> None:
        ...
        if agent_id not in self.agent_capabilities:
            logger.warning(
                f"Agent {agent_id} requested tasks but has no registered capabilities"
            )
            return
