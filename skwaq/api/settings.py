...
SETTINGS = {
    ...
    "api": {
        ...
        "modelDeployments": {"chat": "gpt4o", "code": "o3", "reasoning": "o1"},
    },
    "tools": {
        "codeqlEnabled": True,
...
def test_api_connection():
    ...
    auth_method = (
        "API Key" if api_settings["authMethod"] == "api-key" else "Microsoft Entra ID"
    )
...
def export_env_file():
    ...
    env_content.append(
        f"AZURE_OPENAI_MODEL_DEPLOYMENTS={{\"chat\":\"{model_deployments['chat']}\",\"code\":\"{model_deployments['code']}\",\"reasoning\":\"{model_deployments['reasoning']}\"}}"
    )

    # Return the .env file content
    return jsonify({"status": "success", "env_content": "\n".join(env_content)})
