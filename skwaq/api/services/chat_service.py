    ...
    try:
        ...
        for conversation in CONVERSATIONS.values():
            if conversation.get("userId") == user_id:
                ...
                conversation_copy["messageCount"] = len(MESSAGES.get(conversation["id"], []))
                conversation_copy["lastMessageTime"] = get_latest_message_time(conversation["id"])
                user_conversations.append(conversation_copy)
                
        # Sort by last message time
        user_conversations.sort(
            key=lambda c: c.get("lastMessageTime", c.get("createdAt", "")), 
            reverse=True
        )
...
async def send_system_message(
    ...
    try:
        ...
        logger.info(f"Added system message {message_id} to conversation {conversation_id}")
        
        # Publish event for message sent
        publish_event("chat", "message_sent", {
            "conversationId": conversation_id,
        ...
        logger.error(f"Error sending system message in conversation {conversation_id}: {e}")
        return {
            "id": f"error-{str(uuid.uuid4())}",
            "conversationId": conversation_id,
            "userId": "system",
...
def generate_response(conversation_id: str, user_message: Dict[str, Any]) -> None:
    ...
    try:
        ...
        if "hello" in user_content or "hi" in user_content:
            response_content = "Hello! How can I help you with your security research today?"
        ...
        if conversation_id in MESSAGES:
            MESSAGES[conversation_id] = [msg for msg in MESSAGES[conversation_id] if msg["id"] != thinking_message_id]
        ...
        logger.error(f"Error generating AI response in conversation {conversation_id}: {e}")
        # Send error message
        ...
        if conversation_id in MESSAGES:
            ...
            MESSAGES[conversation_id] = [msg for msg in MESSAGES[conversation_id] if msg.get("isTyping") is not True]
