...
def create_investigation():
    ...
    try:
        ...
        investigation_id = connector.create_node("Investigation", investigation_data)
        ...
        if repository_id:
            ...
            connector.run_query(
                relationship_query,
                {"investigation_id": investigation_id, "repository_id": repository_id},
            )
            ...
            jsonify(
                {
                    ...
                    "repository_name": (
                        "Unknown Repository" if not repository_id else ""
                    ),
                }
            ),
            201,
...
def get_investigation_visualization(investigation_id):
    ...
    try:
        ...
        logger.error(
            f"Error retrieving visualization for investigation {investigation_id}: {str(e)}",
            exc_info=True,
        )
        error_response = {
            "error": str(e),
