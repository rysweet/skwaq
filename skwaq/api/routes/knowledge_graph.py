...
def get_investigation_graph(investigation_id):
    ...
    """
    try:
        include_sources_sinks = request.args.get('include_sources_sinks', 'false').lower() == 'true'
        
        # If the investigation_id is not in the expected format,
        # return a bad request error
        if not validate_investigation_id(investigation_id):
            raise BadRequestError("The string did not match the expected pattern. Investigation ID should be a UUID or start with 'inv-'.")
        ...
        def add_node(node_data):
            ...
            if node_id not in nodes:
                ...
                node_labels = [label for label in node_data.labels if label != 'Resource']
                node_type = node_labels[0].lower() if node_labels else 'unknown'
                
                # Create the node object
                nodes[node_id] = {
                    "id": node_id,
                    "name": node_data.get("name", node_data.get("title", f"Node {node_id}")),
                    "type": node_type,
                    "properties": {
                        **{k: v for k, v in node_data.items() if k not in ['id', 'name', 'title']}
                    }
                }
    ...
    try:
        ...
        for record in result:
            ...
            if node_data:
                ...
                nodes.append({
                    "id": node_data.get("id"),
                    "name": node_data.get("name", node_data.get("title", f"Node {node_data.get('id')}")),
                    "type": node_type,
                    "properties": {
                        **{k: v for k, v in node_data.items() if k not in ['id', 'name', 'title']}
                    }
