...
def get_chat_sessions():
    ...
    sessions = [{k: v for k, v in session.items() if k != 'messages'} for session in CHAT_SESSIONS]
    return jsonify(sessions)

@bp.route('/sessions/<session_id>', methods=['GET'])
def get_chat_session(session_id):
    """Get a specific chat session by ID."""
    session = next((session for session in CHAT_SESSIONS if session['id'] == session_id), None)
    if session is None:
...
def delete_chat_session(session_id):
    ...
    CHAT_SESSIONS = [session for session in CHAT_SESSIONS if session['id'] != session_id]
    return '', 204
...
def generate_response(message):
    ...
    if 'xss' in message.lower() or 'cross' in message.lower() and 'script' in message.lower():
