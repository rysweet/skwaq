...
def visualize_investigation():
    ...
    RETURN elementId(file) as source, elementId(ast) as target
    """
    
    file_ids = [file["id"] for file in file_results]
    ast_ids = [ast["id"] for ast in ast_results]
    
    rel_results = connector.run_query(rel_query, {"file_ids": file_ids, "ast_ids": ast_ids})
    print(f"Found {len(rel_results)} DEFINES relationships")
    ...
    output_path = f"investigation-{investigation_id}-custom.html"
