#!/usr/bin/env python3
"""Create a visualization of AST nodes and AI summaries."""
...
def main():
    """Generate a visualization of AST nodes and AI summaries."""
    connector = get_connector()
    
    # Query for getting all AST nodes with bidirectional relationships to files
    ast_query = """
    MATCH (file:File)<-[:PART_OF]-(ast)
    ...
    print(f"Found {len(ast_results)} AST nodes with PART_OF relationships")
    
    # Get all file IDs from AST results
    file_ids = list(set([ast["file_id"] for ast in ast_results]))
    
    # Get file details
    file_query = """
    MATCH (file:File)
    WHERE elementId(file) IN $file_ids
    RETURN file.name as name, file.path as path, elementId(file) as id
    """
    
    files = connector.run_query(file_query, {"file_ids": file_ids})
    ...
    # Add AST nodes and relationships (both directions)
    ...
    <head>
        <meta charset="utf-8">
        <title>AST and AI Summaries Visualization</title>
    ...
    <body>
        ...
        <div id="controls">
            ...
            <div class="relationship-toggle">
                ...
                <div>
                    <input type="checkbox" id="show-describes" checked>
                    <label for="show-describes">DESCRIBES</label>
                </div>
            </div>
        ...
        <script>
            ...
            const simulation = d3.forceSimulation()
                .force("link", d3.forceLink().id(d => d.id).distance(d => {
                    ...
                    const sourceType = d.source.type || "Unknown";
                    const targetType = d.target.type || "Unknown";
                    
                    if (sourceType === "CodeSummary" || targetType === "CodeSummary") {
                        return 100; // Summaries closer to their nodes
                    } else {
                        return 70; // Default distance
                    }
                }))
            ...
            function showNodeInfo(event, d) {
                    ...
                    for (const [key, value] of Object.entries(d.properties)) {
                        if (value !== null && value !== undefined) {
                            if (key === "summary") {
                                ...
                                html += `<li>${key}: <pre style="white-space: pre-wrap; font-size: 12px; background-color: #f0f0f0; padding: 8px; border-radius: 4px;">${value}</pre></li>`;
                            } else {
                ...
                if (connectedLinks.length > 0) {
                    ...
                    connectedLinks.forEach(l => {
                        ...
                        const otherNode = nodesData.find(n => n.id === otherNodeId);
                        
                        if (otherNode) {
                            const direction = sourceId === d.id ? "→" : "←";
                            html += `<li>${direction} ${l.label} ${otherNode.type}: ${otherNode.label}</li>`;
                        }
                    });
            ...
            document.getElementById("show-describes").addEventListener("change", filterGraph);
            ...
            function filterGraph() {
                ...
                const showDescribes = document.getElementById("show-describes").checked;
                ...
                // Filter links based on relationship types and visible nodes
                link.style("display", d => {
                    // Filter by relationship type
                    if (d.label === "DEFINES" && !showDefines) return "none";
                    if (d.label === "PART_OF" && !showPartOf) return "none";
                    if ((d.label === "DESCRIBES" || d.label === "SUMMARIZES") && !showDescribes) return "none";
                ...
                linkLabel.style("display", d => {
                    ...
                    // Filter by node visibility (same as for links)
                    const sourceType = d.source.type;
                    const targetType = d.target.type;
    ...
    # Write to file
    output_path = "attackbot_ast_summary_visualization.html"
    with open(output_path, "w") as f:
        f.write(html)
    
    print(f"AST and summary visualization created at {output_path}")
    print(f"Nodes: {len(nodes)} ({sum(1 for n in nodes if n['type'] == 'File')} files, "
          ...
          f"{sum(1 for n in nodes if n['type'] == 'CodeSummary')} AI summaries)")
