...
"""
    ...
    async def test_ast_analysis(self):
        ...
        assert (
            len(list(eval_pattern.finditer(python_content))) == 0
        )  # Should not find any

        # Check for SQL injection pattern
        sql_pattern = re.compile(
            r'query\s*=\s*["\'"]SELECT.+[+]', re.MULTILINE | re.IGNORECASE
        )
        ...
        cmd_pattern = re.compile(
            r'subprocess\.(?:call|run|Popen)\s*\(\s*(?:["\'].+["\']\s*\+\s*|[f"\'])',
            re.MULTILINE,
        )
        ...
        xss_pattern = re.compile(
            r'(?:innerHTML|outerHTML)\s*=\s*(?!["\']\s*\+)', re.MULTILINE
        )
        ...
        python_findings = [
            {
                ...
                "vulnerability_type": "SQL Injection",
                "line_number": (
                    python_content[: sql_matches[0].start()].count("\n") + 1
                    if sql_matches
                    else 0
                ),
            ...
            },
            {
                ...
                "vulnerability_type": "Command Injection",
                "line_number": (
                    python_content[: cmd_matches[0].start()].count("\n") + 1
                    if cmd_matches
                    else 0
                ),
    ...
    async def test_analyze_repository(self):
        ...
        repo_results = {
            ...
            "files_analyzed": 2,
            "vulnerabilities_found": python_file_results["vulnerabilities_found"]
            + js_file_results["vulnerabilities_found"],
            "patterns_matched": python_file_results["patterns_matched"]
            + js_file_results["patterns_matched"],
            "analysis_details": [
                {
                    "file_id": 101,
