...
"""
        ...
        category = ET.SubElement(
            root, "Category", ID="1004", Name="Sanitization Issues"
        )
...
"""
        ...
        cve_data = {
            "CVE_Items": [
                {
                    "cve": {
                        "CVE_data_meta": {"ID": "CVE-2021-99999"},
                        "description": {
                            "description_data": [
                                {
                                    "lang": "en",
    ...
    def test_document_structure(self):
        ...
        assert (
            cve_data["CVE_Items"][0]["cve"]["CVE_data_meta"]["ID"] == "CVE-2021-99999"
        )
    ...
    async def _run_knowledge_ingestion_test(
        self, mock_kg, mock_connector, mock_client, knowledge_dir, cwe_file, cve_file
    ):
    ...
    def test_knowledge_chunker_logic(self):
        ...
        assert any(
            section["header"] == "SQL Injection Vulnerability" for section in sections
        )
        assert any(section["header"] == "Prevention" for section in sections)
        assert any(
            section["header"] == "Vulnerable Code Example" for section in sections
        )
...
class TestKnowledgeGraphIntegration:
    ...
    async def _simulate_knowledge_linking(self, connector, threshold):
        ...
        cwe_pattern_result = connector.run_query(
            cwe_pattern_query, {"threshold": threshold}
        )
        cve_pattern_result = connector.run_query(
            cve_pattern_query, {"threshold": threshold}
        )
