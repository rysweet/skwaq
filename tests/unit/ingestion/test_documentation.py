...
@pytest.mark.asyncio
async def test_process_local_docs(
    mock_connector, mock_async_model_client, mock_doc_dir
):
...
@pytest.mark.asyncio
async def test_process_local_docs_nonexistent_path(
    mock_connector, mock_async_model_client
):
...
async def test_process_remote_docs(mock_connector, mock_async_model_client):
    ...
    with patch("aiohttp.ClientSession", return_value=mock_session):
        ...
        result = await processor.process_remote_docs(
            "https://example.com/docs.md", repo_node_id=1
        )
...
async def test_process_remote_docs_error(mock_connector, mock_async_model_client):
    ...
    with patch("aiohttp.ClientSession", return_value=mock_session):
        ...
        with pytest.raises(ValueError) as excinfo:
            await processor.process_remote_docs(
                "https://example.com/docs.md", repo_node_id=1
            )
