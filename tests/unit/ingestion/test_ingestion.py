...
async def test_run_ingestion_local_path(
    ...
    with (
        patch(
            "skwaq.ingestion.ingestion.CodebaseFileSystem", return_value=mock_filesystem
        ),
        patch(
            "skwaq.ingestion.ingestion.FilesystemGraphBuilder"
        ) as mock_fs_builder_cls,
        ...
        mock_fs_builder.build_graph = AsyncMock(
            return_value={"file1.py": 1, "file2.py": 2}
        )
        mock_fs_builder_cls.return_value = mock_fs_builder

        # Setup mock documentation processor
        mock_doc_processor = MagicMock()
        mock_doc_processor.process_local_docs = AsyncMock(
            return_value={"files_processed": 2}
        )
        mock_doc_processor.process_remote_docs = AsyncMock(
            return_value={"files_processed": 1}
        )
...
async def test_run_ingestion_with_documentation(
    ...
    with (
        ...
        mock_doc_processor.process_local_docs.assert_called_once_with(
            "/path/to/docs", 1
        )
        mock_doc_processor.process_remote_docs.assert_called_once_with(
