...
async def test_blarify_parser_initialization():
    ...
    if platform.system() == "Darwin":
        ...
        assert parser.use_docker is False, "Should not use Docker on non-macOS platforms"


@pytest.mark.asyncio
@mock.patch('subprocess.run')
@mock.patch('skwaq.ingestion.parsers.blarify_parser.BlarifyParser._process_docker_result')
async def test_docker_fallback(mock_process_result, mock_subprocess_run, tmpdir):
    ...
    docker_run_result.stdout = json.dumps({
        ...
        "nodes": [
            {"id": 1, "type": "File", "labels": ["File"], "file_path": "test.py", "name": "test.py"},
            {"id": 2, "type": "Function", "labels": ["Function"], "file_path": "test.py", "name": "test_function"}
        ],
        "relationships": [
    ...
    # Configure the mocks
    mock_subprocess_run.side_effect = [docker_check_result, docker_build_result, docker_run_result]
    ...
    assert mock_subprocess_run.call_count == 3, "Should have called subprocess.run 3 times"
    assert mock_process_result.call_count == 1, "Should have processed the Docker result"
