...
class TestGraphVisualizer:
    ...
    def test_get_investigation_graph_with_repository(self, mock_connector, visualizer):
        ...
        investigation_node = next((n for n in result["nodes"] if n["type"] == "investigation"), None)
        assert investigation_node is not None
        assert investigation_node["properties"]["workflow_id"] == "workflow-123"
        
        # Check repository node
        repository_node = next((n for n in result["nodes"] if n["type"] == "repository"), None)
    ...
    def test_get_investigation_graph_with_findings(self, mock_connector, visualizer):
        ...
        # Check finding node
        finding_node = next((n for n in result["nodes"] if n["type"] == "finding"), None)
        ...
        finding_link = next((l for l in result["links"] if l["target"] == "f-789"), None)
    ...
    def test_export_graph_as_html(self, visualizer):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch.object(visualizer, "export_graph_as_html", return_value=output_path) as mock_export:
