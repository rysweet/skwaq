...
def test_get_workflows(mock_get, client, auth_token):
    ...
    with patch("skwaq.api.routes.workflows.asyncio.run", return_value=MOCK_WORKFLOWS):
        response = client.get(
            "/api/workflows", headers={"Authorization": f"Bearer {auth_token}"}
        )
...
def test_get_workflow(mock_get, client, auth_token):
    ...
    # Call the route
    with patch(
        "skwaq.api.routes.workflows.asyncio.run", return_value=MOCK_WORKFLOWS[0]
    ):
        response = client.get(
...
def test_get_repository_workflows(mock_get, client, auth_token):
    ...
    # Call the route
    with patch(
        "skwaq.api.routes.workflows.asyncio.run",
        return_value=[MOCK_EXECUTION, MOCK_EXECUTION_COMPLETED],
    ):
        response = client.get(
...
def test_cancel_workflow_execution(mock_cancel, mock_get, client, auth_token):
    ...
    with patch(
        "skwaq.api.routes.workflows.asyncio.run",
        side_effect=[execution, True, execution_cancelled],
    ):
        response = client.post(
...
def test_cancel_workflow_execution_already_completed(mock_get, client, auth_token):
    ...
    # Call the route
    with patch(
        "skwaq.api.routes.workflows.asyncio.run", return_value=MOCK_EXECUTION_COMPLETED
    ):
        response = client.post(
...
def test_get_workflow_results(mock_results, mock_get, client, auth_token):
    ...
    with patch(
        "skwaq.api.routes.workflows.asyncio.run",
        side_effect=[MOCK_EXECUTION_COMPLETED, MOCK_WORKFLOW_RESULTS],
    ):
        response = client.get(
...
def test_execute_workflow(mock_execute, mock_workflow, mock_repo, client, auth_token):
    ...
    with patch(
        "skwaq.api.routes.workflows.asyncio.run",
        side_effect=[MOCK_REPOSITORY, MOCK_WORKFLOWS[0], True],
    ):
        response = client.post(
...
def test_execute_workflow_missing_data(client, auth_token):
    ...
    response = client.post(
        "/api/workflows/execute",
        headers={"Authorization": f"Bearer {auth_token}"},
        json={"workflowType": "vulnerability_assessment"},
    )
    assert response.status_code == 400
    data = json.loads(response.data)
    assert "error" in data
...
def test_execute_workflow_type_not_found(mock_workflow, mock_repo, client, auth_token):
    ...
    # Call the route
    with patch(
        "skwaq.api.routes.workflows.asyncio.run", side_effect=[MOCK_REPOSITORY, None]
    ):
        response = client.post(
            "/api/workflows/execute",
            headers={"Authorization": f"Bearer {auth_token}"},
            json={"workflowType": "nonexistent", "repositoryId": "repo-123"},
        )
