...
def test_delete_conversation(mock_delete, mock_get, client, auth_token):
    ...
    # Call the route
    with patch('skwaq.api.routes.chat.asyncio.run', side_effect=[MOCK_CONVERSATION, True]):
...
def test_get_messages(mock_messages, mock_get, client, auth_token):
    ...
    # Call the route
    with patch('skwaq.api.routes.chat.asyncio.run', side_effect=[MOCK_CONVERSATION, MOCK_MESSAGES]):
        response = client.get(
...
def test_send_message(mock_send, mock_get, client, auth_token):
    ...
    # Call the route
    with patch('skwaq.api.routes.chat.asyncio.run', side_effect=[MOCK_CONVERSATION, MOCK_USER_MESSAGE]):
        response = client.post(
            '/api/chat/conversations/conv-123/messages',
            headers={'Authorization': f'Bearer {auth_token}'},
            json={
