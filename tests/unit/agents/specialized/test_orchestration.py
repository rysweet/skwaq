...
class TestWorkflowGeneration:
    ...
    async def test_generate_workflow_components_guided_assessment(self, mock_base_init):
        ...
        assert len(components["stages"]) >= 3  # Should have at least initialization, assessment, reporting
    ...
    async def test_generate_workflow_components_comprehensive(self, mock_base_init):
        ...
        assert "stages" in components
        assert len(components["stages"]) >= 7  # Should have multiple stages for comprehensive workflow
        
        assert "communication_patterns" in components
        assert len(components["communication_patterns"]) >= 4  # Should have all communication patterns
        assert "chain_of_thought" in components["communication_patterns"]
        ...
        assert has_collaborative_stage, "Should have at least one collaborative stage with multiple agents"
...
class TestWorkflowExecution:
    ...
    async def test_execute_workflow_sequential_stages(self):
        ...
        class MockAdvancedOrchestrator:
            def __init__(self):
                ...
                self._compile_workflow_results = MagicMock(return_value={"summary": "workflow results"})
    ...
    async def test_execute_workflow_with_dependencies(self):
                ...
                for i, stage in enumerate(workflow_def.stages):
                    ...
                    if "dependencies" in stage:
                        for dep in stage["dependencies"]:
                            ...
                            dep_index = next((idx for idx, s in enumerate(workflow_def.stages) 
                                             if s["name"] == dep), None)
                            if dep_index is None or dep_index not in self.executed_stages:
        ...
        workflow_def = WorkflowDefinition(
            ...
            stages=[
                {"name": "stage1", "agent": "guided_assessment", "description": "Stage 1"},
                {"name": "stage2", "agent": "guided_assessment", "description": "Stage 2", "dependencies": ["stage1"]},
                {"name": "stage3", "agent": "exploitation_verification", "description": "Stage 3", "dependencies": ["stage1"]},
                {"name": "stage4", "agent": "guided_assessment", "description": "Stage 4", "dependencies": ["stage2", "stage3"]}
            ],
            communication_patterns=["chain_of_thought"],
            created_at=1000000
        )
    ...
    async def test_compile_workflow_results(self, mock_base_init):
        ...
        workflow_def = WorkflowDefinition(
            ...
            target_type="repository",
            parameters={},
            agents=["guided_assessment", "exploitation_verification", "remediation_planning", "security_policy"],
            stages=[
                {"name": "initialization", "agent": "guided_assessment", "description": "Initialize workflow"},
                {"name": "assessment", "agent": "guided_assessment", "description": "Assessment"},
                {"name": "exploitation", "agent": "exploitation_verification", "description": "Exploitation verification"},
                {"name": "remediation", "agent": "remediation_planning", "description": "Remediation planning"},
                {"name": "policy", "agent": "security_policy", "description": "Policy compliance"}
            ],
        ...
        workflow_exec.artifacts = {
            ...
            "findings": [
                {"file_id": "finding_1", "vulnerability_type": "SQL Injection", "severity": "high"},
                {"file_id": "finding_2", "vulnerability_type": "XSS", "severity": "medium"}
            ],
            "verifications": [
                {"verification_id": "verification_1", "finding_id": "finding_1", "status": "exploitable"},
                {"verification_id": "verification_2", "finding_id": "finding_2", "status": "potentially_exploitable"}
            ],
            "remediation_plans": [
                {"plan_id": "plan_1", "finding_id": "finding_1", "priority": "high", "complexity": "moderate"},
                {"plan_id": "plan_2", "finding_id": "finding_2", "priority": "medium", "complexity": "simple"}
            ],
            "policy_evaluation": {
                "evaluation_id": "eval_1",
                "compliance_status": "partially_compliant",
                "compliance_gaps": [{"category": "input_validation", "description": "Missing input validation"}]
            }
        }
