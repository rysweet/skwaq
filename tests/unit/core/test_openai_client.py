...
async def test_get_completion():
    ...
    with patch("openai.OpenAI"), patch("openai.AzureOpenAI"), patch.object(OpenAIClient, "client", create=True) as mock_client:
...
def test_azure_entra_id_auth():
    ...
    with patch("skwaq.core.openai_client.AzureOpenAI") as mock_azure_openai, \
         patch("skwaq.core.openai_client.OpenAI"), \
...
def test_client_registry():
    ...
    with patch("skwaq.core.openai_client.AzureOpenAI") as mock_azure_openai, \
        ...
        with patch("skwaq.core.openai_client.OpenAIClient", side_effect=[client1, client2, client3]):
