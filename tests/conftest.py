...
def reset_registries_and_modules(monkeypatch):
    ...
    try:
        patterns_registry_module = importlib.import_module(
            "skwaq.code_analysis.patterns.registry"
        )
    ...
    if code_ingestion_module is not None and hasattr(
        code_ingestion_module, "RepositoryIngestor"
    ):
        monkeypatch.setattr(
            code_ingestion_module.RepositoryIngestor,
            "_init_github_client",
            safe_init_github_client,
        )
    ...
    try:
        ...
        if hasattr(analyzer_module, "CodeAnalyzer") and hasattr(
            analyzer_module.CodeAnalyzer, "_instance"
        ):
            analyzer_module.CodeAnalyzer._instance = None

        # Reset RepositoryIngestor singleton if it exists
        if hasattr(code_ingestion_module, "RepositoryIngestor") and hasattr(
            code_ingestion_module.RepositoryIngestor, "_instance"
        ):
    ...
    if patterns_registry_module is not None and hasattr(
    ...
    mock_openai_client.get_completion = MagicMock(
        return_value="Mock repository summary"
    )
    # For async tests
    mock_async_openai_client = MagicMock()
    mock_async_openai_client.get_completion = AsyncMock(
        return_value="Mock repository summary"
    )
            ...
            for name in dir(module):
                try:
                    ...
                    if isinstance(cls, type) and hasattr(cls, "_instance"):
                        ...
                        logger.debug(
                            f"Reset singleton instance for {module_name}.{name}"
                        )
...
def mock_git_everywhere(monkeypatch):
    ...
    git_module_mock.InvalidGitRepositoryError = type(
        "InvalidGitRepositoryError", (Exception,), {}
    )
...
def isolated_test_environment(monkeypatch):
    ...
    github_module_mock.GithubException = type(
        "IsolatedGithubException", (Exception,), {}
    )
    ...
    mock_openai_client.create_completion = AsyncMock(
        return_value={"choices": [{"text": "Isolated test summary"}]}
    )
