...
def test_investigation(client, neo4j_test_connection) -> Dict[str, Any]:
    ...
    try:
        with driver.session(database=connection_details["database"]) as session:
            ...
            """
            repo_result = session.run(repo_query, {
            ...
            investigation_result = session.run(investigation_query, {
            ...
            finding_result = session.run(finding_query, {
                "investigation_id": investigation_id,
            ...
            vuln_result = session.run(vuln_query, {
                "finding_id": finding_id,
