...
def neo4j_test_repo():
    ...
    repo_node_id = connector.create_node(
        NodeLabels.REPOSITORY, repo_properties
    )
...
async def test_llm_summarizer_integration(test_codebase_fs, neo4j_test_repo):
    ...
    class TestOpenAIClient:
        ...
        def __init__(self, config):
            ...
            if config.openai.get("use_entra_id"):
                ...
                token_scope = config.openai.get("token_scope", "https://cognitiveservices.azure.com/.default")
                
                # We'll use token provider to get the token just-in-time 
                # for each request to ensure it's fresh
                self.azure_credential = azure_credential
                self.token_scope = token_scope
                openai_config["azure_ad_token_provider"] = lambda: azure_credential.get_token(token_scope).token
            else:
        ...
        async def get_completion(self, prompt, **kwargs):
            ...
            try:
                ...
                if "temperature" in str(e) or "max_tokens" in str(e):
                    print(f"Error with basic parameters: {str(e)}. Retrying with enhanced parameters...")
                    ...
                    try:
                        ...
                        print(f"Error with enhanced parameters: {str(e)}. Retrying with minimal parameters...")
    ...
    for file_path in test_codebase_fs.get_all_files():
        ...
        file_node_id = connector.create_node(
            NodeLabels.FILE, file_properties
        )
        ...
        """
        connector.run_query(rel_query, {
            "repo_id": neo4j_test_repo["repo_id"],
    ...
    # Run the summarizer
    result = await summarizer.summarize_files(file_nodes, test_codebase_fs, neo4j_test_repo["repo_id"])
    ...
    assert result["stats"]["errors"] == 0, f"Should not have errors: {result.get('errors', [])}"
    
    # Check that all files were processed
    assert result["files_processed"] == len(file_nodes), f"Expected {len(file_nodes)} files processed, got {result['files_processed']}"
    ...
    summary_results = connector.run_query(summary_query, {"test_id": neo4j_test_repo["test_id"]})
    
    # Verify that summaries were created for all files
    assert len(summary_results) == len(file_nodes), f"Expected {len(file_nodes)} summaries, got {len(summary_results)}"
    ...
    for result in summary_results:
        ...
        if "main.py" in file_path:
            assert any(term in summary.lower() for term in ["add", "subtract"]), "Summary should mention key functions"
        elif "helpers.py" in file_path:
            assert "calculator" in summary.lower(), "Summary should mention Calculator class"
        
        # Verify summary is non-empty and substantial
        assert len(summary) > 100, f"Summary for {file_path} should be substantial (>100 chars)"
...
""",
    ...
    async def run_test():
        ...
        try:
            await test_llm_summarizer_integration(fs, {"repo_id": repo_node_id, "test_id": test_id})
        finally:
            # Clean up test nodes
