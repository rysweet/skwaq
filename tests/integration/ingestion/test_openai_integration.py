...
async def test_openai_client_direct_completion(openai_api_credentials: Dict[str, str]):
    ...
    config = Config(
        ...
        openai_org_id=openai_api_credentials.get("org_id", "")  # Use empty string if org_id not provided
    )
    ...
    if openai_api_credentials.get("api_type") == "azure":
        ...
        if openai_api_credentials.get("model_deployments"):
            openai_config["model_deployments"] = openai_api_credentials.get("model_deployments")
        ...
        if openai_api_credentials.get("use_entra_id"):
            ...
            if openai_api_credentials.get("auth_method") == "bearer_token":
                ...
                if openai_api_credentials.get("token_scope"):
                    openai_config["token_scope"] = openai_api_credentials.get("token_scope")
            else:
                ...
                if openai_api_credentials.get("client_secret"):
                    openai_config["client_secret"] = openai_api_credentials.get("client_secret")
    ...
    assert "add" in response.lower() or "sum" in response.lower(), "Response doesn't mention function purpose"
...
async def test_llm_summarizer_with_real_client(
    ...
    TestFileSystem = test_file_system  # Get the actual TestFileSystem class from the fixture
...
async def test_openai_client_registry():
    ...
    assert client1 is client2, "Client registry did not return the same client for the same key"
    
    # Verify that client1 and client3 are different objects
    assert client1 is not client3, "Client registry returned the same client for different keys"
