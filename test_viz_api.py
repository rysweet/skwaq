        ...
        if response.status_code == 200:
            try:
                ...
                for i, node in enumerate(data.get('nodes', [])[:3]):
                    print(f'  {i+1}. {node.get("type")}: {node.get("label")} (ID: {node.get("id")})')
                
                # Print a few sample links
                print('\nSample links:')
                for i, link in enumerate(data.get('links', [])[:3]):
                    print(f'  {i+1}. {link.get("source")} -> {link.get("target")} ({link.get("type")})')
                    
                # Check if any links have the investigation node as source
                inv_links = [link for link in data.get('links', []) if isinstance(link.get('source'), str) and link.get('source') == investigation_id]
