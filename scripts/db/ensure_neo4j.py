...
def seed_mock_data():
    ...
    try:
        ...
        for inv in investigations:
            ...
            repo_result = connector.run_query(repo_query, {"repo_id": inv["repository_id"]})
        ...
        for finding in findings:
            ...
            if finding_id:
                ...
                connector.create_relationship(
                    finding_id, inv_id, "BELONGS_TO", {"timestamp": "2023-01-02T12:00:00Z"}
                )
                ...
                file_props = {
                    ...
                    "language": "python" if finding["file_path"].endswith(".py") else "html"
                }
                
                file_id = connector.create_node("File", file_props)
                if file_id:
                    connector.create_relationship(
                        finding_id, file_id, "FOUND_IN", {}
                    )
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Ensure Neo4j is running and initialized")
    parser.add_argument("--seed", action="store_true", help="Seed the database with mock data")
    parser.add_argument("--clear", action="store_true", help="Clear all data from the database before initializing")
    parser.add_argument("--force-restart", action="store_true", help="Force restart Neo4j container even if it's running")
