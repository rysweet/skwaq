...
def start_server(host="localhost", port=5000, debug=False, test_mode=False):
    ...
    logger.info(f"Starting {'TEST' if test_mode else 'API'} server with command: {' '.join(cmd)}")
    ...
    if process.poll() is None:
        logger.info(f"{'TEST' if test_mode else 'API'} server started with PID {process.pid}")
        return True
    else:
        stdout, stderr = process.communicate()
        logger.error(f"Failed to start {'TEST' if test_mode else 'API'} server. Error: {stderr}")
        return False
...
if __name__ == "__main__":
    ...
    start_parser.add_argument('--test', action='store_true', help='Run in test mode with authentication disabled')
    ...
    restart_parser.add_argument('--port', type=int, default=5000, help='Port to bind to')
    restart_parser.add_argument('--debug', action='store_true', help='Run in debug mode')
    restart_parser.add_argument('--test', action='store_true', help='Run in test mode with authentication disabled')
    
    # Status command
    status_parser = subparsers.add_parser('status', help='Check if the API server is running')
