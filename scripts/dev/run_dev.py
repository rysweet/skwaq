...
def run_frontend(port, api_port):
    ...
    print(
        f"\033[1;32m[FRONTEND]\033[0m Starting frontend server at http://localhost:{port}"
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run the Skwaq development environment"
    )
    parser.add_argument(
        "--api-host", default="127.0.0.1", help="API host (default: 127.0.0.1)"
    )
    parser.add_argument(
        "--api-port", type=int, default=5000, help="API port (default: 5000)"
    )
    parser.add_argument(
    ...
    try:
        ...
        frontend_thread = Thread(
            target=print_process_output, args=(frontend_process, "[FRONTEND]")
        )
