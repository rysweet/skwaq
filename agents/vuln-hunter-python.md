...
**CWE-89 SQL Injection (Python) — Complete Source-Sink Analysis:** Trace all user input to SQL execution. Sources: `request.args`, `request.form`, `request.json`, `sys.argv`, `input()`, `os.environ`, file reads. Sinks: `cursor.execute()`, `cursor.executemany()`, `connection.execute()`, `engine.execute()`, `session.execute()`, Django `raw()`, SQLAlchemy `text()`. Unsafe query building: f-strings (`f"SELECT...{var}"`), `.format()` (`"SELECT...{}".format(var)`), `%` operator (`"SELECT...%s" % var`), `+` concatenation (`"SELECT..." + var`). Safe alternatives: parameterized queries (`cursor.execute("SELECT...WHERE id=?", (var,))`), ORM methods. The `.format()` and `%` patterns are especially dangerous because they look like parameterization but are NOT — they build the query string before the database sees it.

**CWE-312/798 Hardcoded Credentials (Python):** Detect private keys (`-----BEGIN RSA PRIVATE KEY-----`), API keys, passwords, and tokens hardcoded in source. Check for: string literals assigned to variables named `key`, `secret`, `password`, `token`, `api_key`; base64-encoded credential blobs; AWS access keys (`AKIA...`). Safe alternatives: environment variables, key management services, config files excluded from version control.
