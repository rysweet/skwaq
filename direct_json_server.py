    ...
    def do_GET(self):
        ...
        if match:
            ...
            try:
                ...
                print(
                    f"Served JSON data with {len(data.get('nodes', []))} nodes and {len(data.get('links', []))} links"
                )
            ...
            self.wfile.write(
                json.dumps(
                    {
                        "message": "Use the /api/investigations/{id}/visualization endpoint"
                    }
                ).encode()
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Serve CLI-generated investigation visualization JSON"
    )
    ...
    try:
        with open(args.json_file, "r") as f:
            data = json.load(f)
            print(
                f"Loaded JSON file with {len(data.get('nodes', []))} nodes and {len(data.get('links', []))} links"
            )
    except Exception as e:
    ...
    print(
        f"Use http://localhost:{args.port}/api/investigations/any-id/visualization to access the data"
    )
